...
namespace TestProject1
{
    ...
    public class TC15 : BaseWebAiiTest
    {
...
public void TC15_CodedStep()
{
    ...
    {
        Log.WriteLine(myList[i].Cells[6].InnerText.ToString());
        string cellValue = myList[i].Cells[6].InnerText.Trim();
    ...
    bool isSortedDescending = IsSortedDescending(cellValuesAsInt);
    if (isSortedDescending)
    {
        Log.WriteLine("Các giá trị số đã được sắp xếp theo thứ tự giảm dần.");
    }
    else
    {
        Log.WriteLine("Các giá trị số không được sắp xếp theo thứ tự giảm dần.");
    }
}

private bool IsSortedDescending(List<int> values)
{
